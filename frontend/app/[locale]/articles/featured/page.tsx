...
export default async function FeaturedArticlesPage({ params }: { params: { locale: string } }) {
  const dict = getDictionary(params.locale);
  let items: Article[] = [];

  try {
    const response = await apiFetch<PaginatedResponse<Article>>("/articles/articles/");
    items = response.results.slice(0, 6);
  } catch {
    items = [];
  }
    ...
    <div className="bg-[#F3F4F6]">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 sm:px-10">
        {items.length ? (
          ...
          <p className="text-sm text-stone-500">No featured articles yet.</p>
        )}
      </section>
    </div>
  );
